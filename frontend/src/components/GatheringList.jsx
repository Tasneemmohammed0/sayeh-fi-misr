...
function GatheringList({search,filter,count=100}) {
  ...
  if(search){
    gatherings=gatherings.filter((item)=>item.name.toLowerCase().includes(search.toLowerCase()))
  }
    ...
    <div className={styles.list}>
      {
        ...
        <Card key={index} photo={item.image} placeName={item.name } location={item.city} rate={item.rate} type='gathering' />
        </div>)
